...
<div class="card-body">
  ...
  <div class="row">
    <div class="col-lg-9">
      ...
      <div class="form-group row">
        <strong class="col-md-3">{{ __('admin::lang.permissions') }}<span class="text-danger"> *</span> 
        ...
        @endforeach
        <strong class="col-md-1">{{ __('admin::lang.all') }}</strong>
      </div>
      @foreach ($allPages as $page)
        @if($page != 'shops' && $page != 'complaints' && $page != 'notifications' && $page != 'ratings' && $page !='subscriptions'  )
      
        <div class="form-group row">
          <label class="col-md-3 col-form-label">{{ __('admin::lang.'. $page) }}</label>
          @foreach ($allActions as $action)
            @php
